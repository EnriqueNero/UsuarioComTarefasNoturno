...
public class AdicionarCategoria extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (!"admin".equalsIgnoreCase(usuarioLogado.getTipo())) {
            out.print("<p>Apenas administradores podem adicionar categorias.</p>");
            out.print("<br><a href='menu.html'><button>Voltar</button></a>");
            return;
        }
        ...
        int novoId = categorias.stream().mapToInt(Categoria::getId).max().orElse(0) + 1;
        Categoria nova = new Categoria(novoId, nome);
        if (categorias.contains(nova)) {
            out.print("<p>Categoria já existe.</p>");
            out.print("<br><a href='menu.html'><button>Voltar</button></a>");
            return;
        }
        categorias.add(nova);
        out.print("<p>Categoria adicionada com sucesso!</p>");
        out.print("<br><a href='menu.html'><button>Voltar</button></a>");
    }
}